...
public class NadirReforgingMenu extends BlockEntityMenu<NadirReforgingTableTile> {
    ...
    public NadirReforgingMenu(int id, Inventory inv, BlockPos pos) {
        super(Nad.Menus.NADIR_REFORGING, id, inv, pos);
    ...
    public class ReforgingResultSlot extends SlotItemHandler {
        ...
        public boolean mayPickup(Player playerIn) {
            ItemStack input = NadirReforgingMenu.this.getSlot(0).getItem();
            LootRarity rarity = NadirReforgingMenu.this.getRarity();
            ReforgingRecipe recipe = NadirReforgingMenu.this.tile.getRecipeFor(rarity);
            if (recipe == null || input.isEmpty()) return false;

            int sigils = NadirReforgingMenu.this.getSigilCount();
            int sigilCost = NadirReforgingMenu.this.getSigilCost(this.getSlotIndex());
            int mats = NadirReforgingMenu.this.getMatCount();
            int matCost = NadirReforgingMenu.this.getMatCost(this.getSlotIndex());
            int levels = NadirReforgingMenu.this.player.experienceLevel;
            int levelCost = NadirReforgingMenu.this.getLevelCost(this.getSlotIndex());

            if ((sigils < sigilCost || mats < matCost || levels < levelCost) && !NadirReforgingMenu.this.player.isCreative()) return false;
        ...
        public void onTake(Player player, ItemStack stack) {
            if (!player.level().isClientSide) {
                NadirReforgingMenu.this.getSlot(0).set(ItemStack.EMPTY);
                if (!player.isCreative()) {
                    int sigilCost = NadirReforgingMenu.this.getSigilCost(this.getSlotIndex());
                    int matCost = NadirReforgingMenu.this.getMatCost(this.getSlotIndex());
                    int levelCost = NadirReforgingMenu.this.getLevelCost(this.getSlotIndex());
                    NadirReforgingMenu.this.getSlot(1).getItem().shrink(matCost);
                    NadirReforgingMenu.this.getSlot(2).getItem().shrink(sigilCost);
