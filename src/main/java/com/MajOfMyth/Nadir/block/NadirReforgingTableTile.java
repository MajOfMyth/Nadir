...
public class NadirReforgingTableTile extends BlockEntity implements TickingBlockEntity {
    ...
    public InternalItemHandler inv = new InternalItemHandler(2){
        ...
        public boolean isItemValid(int slot, ItemStack stack) {
            if (slot == 0) {
                return NadirReforgingTableTile.this.isValidRarityMat(stack);
            }
        ...
        protected void onContentsChanged(int slot) {
            NadirReforgingTableTile.this.setChanged();
        };
    };

    public NadirReforgingTableTile(BlockPos pWorldPosition, BlockState pBlockState) {
        super(Nad.Tiles.NADIR_REFORGING_TABLE, pWorldPosition, pBlockState);
    }
    ...
    public ReforgingRecipe getRecipeFor(LootRarity rarity) {
        return this.level.getRecipeManager().getAllRecipesFor(Apoth.RecipeTypes.REFORGING)
                .stream()
